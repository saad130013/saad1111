...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            # عرض البيانات المصنفة للمساعدة في التشخيص
            # st.subheader("البيانات المصنفة (للتشخيص)")
            # st.dataframe(df[['التاريخ', 'التفاصيل', 'مدين', 'دائن', 'الحساب المحاسبي']].head(10), use_container_width=True)
